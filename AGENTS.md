...
- 이 레포는 **기존 운영 중인 모노레포**를 기본 전제로 한다. 따라서 **신규 고객사 전체 프로젝트를 처음부터 수행**하는 요청이 아닐 때(예: 기존 계약 범위 내 기능 추가·버그 수정·운영 개선)는 `client-project-lifecycle`을 기본 강제로 적용하지 않고, 요청 성격에 맞는 `start-feature`/`bugfix-flow`/`verify-change`를 우선한다.
