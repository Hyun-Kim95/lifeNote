...
**lifeNote 제품 레포 전제:** `docs/requirements/`, `docs/design/`, API 계약 문서 등이 이미 존재하는 **진행 중 제품**으로 본다. 요구사항을 붙여 넣었다는 이유만으로 전체 `client-project-lifecycle`(PRD 승인 전 구현·목업 코드 중단 포함)을 **자동 적용하지 않는다**. 외주·그린필드이거나 사용자가 **고객 납품·전체 파이프라인**을 명시한 경우에만 해당 스킬 전체를 따른다. 일상 이슈·버그·소규모 기능은 `bugfix-flow`·`start-feature`·사용자의 범위 지시를 우선한다.
