...
if (-not [string]::IsNullOrWhiteSpace($ScriptPath)) {
    ...
    $hookContent += ('powershell -NoProfile -ExecutionPolicy Bypass -File "{0}" -RepoRoot "$REPO_ROOT"' -f $normalizedScriptPath)
    $hookContent += 'powershell -NoProfile -ExecutionPolicy Bypass -File "$REPO_ROOT/scripts/obsidian/sync-docs.ps1"'
} else {
    $hookContent += 'powershell -NoProfile -ExecutionPolicy Bypass -File "$REPO_ROOT/scripts/obsidian/write-commit-journal.ps1" -RepoRoot "$REPO_ROOT"'
    $hookContent += 'powershell -NoProfile -ExecutionPolicy Bypass -File "$REPO_ROOT/scripts/obsidian/sync-docs.ps1"'
}

Set-Content -LiteralPath $hookFile -Value ($hookContent -join "`n") -Encoding ASCII
Write-Host "Hook installed: $hookFile"
Write-Host "On each commit: commit journal -> vault .../journal, then sync-docs -> vault .../docs (see .obsidian-ingest.json for slug/vaultRoot)."
