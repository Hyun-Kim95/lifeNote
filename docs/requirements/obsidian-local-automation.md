...
### macOS/Linux·PowerShell 미설치 환경에서 Cursor hooks 비활성화

`.cursor/hooks.json`은 Windows `powershell.exe`로 `.ps1`를 호출한다. macOS/Linux에서는 `powershell`이 없을 수 있어 Cursor 훅이 오류 로그를 남기거나 세션 시작이 거슬릴 수 있다.

**팀 선택:** 해당 환경에서는 레포 루트의 `.cursor/hooks.json`을 Cursor가 읽지 않게 하면 된다. 예: 파일명을 `hooks.json.disabled`로 바꾸거나, 내용을 `"version": 1, "hooks": {}`처럼 빈 객체로 둔다(Windows에서 Obsidian 자동화를 쓰는 팀원만 로컬에서 원래 설정을 유지해도 된다). 문서 동기화·커밋 저널은 위 **3)·4)**의 수동 `powershell`/`pwsh` 실행으로 동일하게 수행할 수 있다.
