...
- `D:\Obsidian\projects\<slug>\docs\_project-doc-index.md` 자동 생성
...
`docs/obsidian`을 제외한 `docs/requirements`, `docs/qa`, `docs/design`, `docs/decisions`, `docs/changelog`의 마크다운에, **이미 `---`로 시작하지 않는 파일만** 대상으로 YAML frontmatter(`type: doc`, `project`, `doc_lane`, `updated_at`, `tags`)와 하단 `## Vault` 위키링크(허브·대시보드·저널)를 추가한다.

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\obsidian\normalize-doc-frontmatter.ps1"
```

`project` 슬러그는 `.obsidian-ingest.json`의 `slug`가 있으면 그 값을, 없으면 Git 루트 폴더명을 쓴다. Vault 링크 표시 텍스트는 Windows 콘솔 인코딩 호환을 위해 ASCII로 둔다.
...
- 프로젝트 허브는 `[[<project>/docs/_project-doc-index]]`를 기준으로 링크한다.
