...
| Web 통합 E2E (기본) | `npm run e2e -w web` | 4 passed, 1 skipped (모킹 + 인증 시나리오) |
| Web 통합 E2E (실 API) | `E2E_ACCESS_TOKEN=<token> npm run e2e:live -w web` | 1 passed |
...
| CI Secret | GitHub Actions Secret | `E2E_ACCESS_TOKEN` (있을 때만 live E2E 실행) |
...
- 웹: 반응형(모바일 폭), 다크모드(시스템/브라우저), 주요 화면 로딩·빈·오류 상태
