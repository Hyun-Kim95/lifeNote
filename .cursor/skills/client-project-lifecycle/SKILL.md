...
**lifeNote 참고:** 이 스킬은 **고객사 그린필드·전체 납품**을 주 대상으로 한다. `docs/requirements/`, `docs/design/`, API 계약이 이미 있는 제품 레포에서는 [AGENTS.md](../../../AGENTS.md)와 `.cursor/rules/70-client-lifecycle-default.mdc`의 **적용 제외·우선순위**를 먼저 따른다. 사용자가 **고객 납품·전체 파이프라인**을 명시한 경우에만 아래 단계를 순서대로 적용한다.
