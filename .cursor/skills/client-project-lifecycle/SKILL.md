...
- 기존 제품의 증분 개발로서 **Gate 1 이후 전제**가 명확한 경우(기존 계약·화면·API 틀 유지 범위), 단계 0~3 전체를 반복하지 않고 필요한 단계(예: `start-feature`, `bugfix-flow`, 단계 4~7 검증/문서화)만 선택 적용할 수 있다. 단, 대외 API·정책·핵심 UX 계약이 바뀌면 Gate 점검 구간으로 되돌린다.
